  ...
  where TTarget : class, IEquatable<TTarget>, IEquatable<int>, IComparable<TTarget>
{
  ...
  private readonly HashSet<AbsenceTypeRelation<TSource, TTarget>> _relations;
  ...
  )
  {
    ...
    _relations = new HashSet<AbsenceTypeRelation<TSource, TTarget>>();
  ...
  private void Update()
  {
    ...
    {
      var source = _sourceAbsenceTypes.SingleOrDefault(absence => absence.Equals(configuredRelation.IdOfSourceSystem));
      var target = _targetAbsenceTypes.SingleOrDefault(absence => absence.Equals(configuredRelation.IdOfTargetSystem));
  ...
  {
    var result = _relations.FirstOrDefault(relation => relation.Source == sourceAbsence)?.Export;
  ...
  public (bool doExport, TTarget? toType) CheckExport(TSource source)
  {
    ...
    if (relation == null)
      throw new InvalidOperationException($"Call {nameof(NotMappedFromSourceToTarget)} first");
