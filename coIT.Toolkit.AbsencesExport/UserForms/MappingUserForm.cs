  ...
  where TTarget : class, IEquatable<TTarget>, IEquatable<int>, IComparable<TTarget>
{
  ...
  private void UpdateConfigurationFiles(List<ExportRelations> configurations)
  {
    ...
    {
      cbxConfigurationFiles.Invoke(() => UpdateConfigurationFiles(configurations));
    }
    else
    {
  ...
  private void UpdateListBoxes()
  {
    ...
    lbxAbsenceTypesSourceSystem.Items.AddRange(_relations.NotMappedFromSourceToTarget().ToArray());

    lbxIgnoredRelations.Items.Clear();
    lbxIgnoredRelations.Items.AddRange(_relations.MappedButIgnored().OrderBy(relation => relation.Source).ToArray());
  ...
  private void btnCrateMapping_Click(object sender, EventArgs e)
  {
    ...
    maybeTargetAbsenceType
      .Map(_ =>
        AbsenceTypeRelation<TSource, TTarget>.Create(maybeSourceAbsenceType.Value, maybeTargetAbsenceType.Value)
      )
      .Execute(relation =>
      {
  ...
  {
    var maybeRelationWithoutExportSelected = MaybeSelected<AbsenceTypeRelation<TSource, TTarget>>(
      lbxRelationsWithoutExport
    );
    var maybeRelationWithExportSelected = MaybeSelected<AbsenceTypeRelation<TSource, TTarget>>(lbxRelationsWithExport);
    ...
    maybeRelationWithoutExportSelected.Or(maybeRelationWithExportSelected).Execute(_ => ListBoxesChanged());
  }
  ...
  private async void btnStoreConfig_Click(object sender, EventArgs e)
  {
    ...
    await _exportRelationRepository.Save(configuration).Tap(UpdateConfigurationFileList).TapError(DisplayError);
  ...
  private void AddTextWrap(ListBox listBox)
  {
    ...
    {
      e.ItemHeight = (int)
        e.Graphics.MeasureString(listBox.Items[e.Index].ToString(), listBox.Font, listBox.Width).Height;
    }
  ...
  private async void btnNewConfig_Click(object sender, EventArgs e)
  {
    ...
    var relations = new ExportRelations
    {
      ...
      Relations = new HashSet<ExportRelation>(),
    };
