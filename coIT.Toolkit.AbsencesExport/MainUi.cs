  ...
  where TTarget : class, IEquatable<TTarget>, IEquatable<int>, IComparable<TTarget>
{
  ...
  private readonly AktivierteFilter<TSource, TTarget> _aktivierteMitarbeiterFilter = new();
  ...
  {
    InitializeComponent();
    ((Control)tbpAnzeige).Enabled = false;
  ...
  private async void MainUi_Load(object sender, EventArgs e)
  {
    ...
    ((Control)tbpAnzeige).Enabled = true;
  }

  private void MitarbeiterListeAktualisieren(IImmutableList<Abwesenheitseintrag<TSource, TTarget>> abwesenheiten)
  {
    clbxMitarbeiterFilter.Items.Clear();

    var auswahl = abwesenheiten.Select(a => (a.Name, a.Personalnummer)).Distinct().OrderBy(t => t.Name).ToList();

    auswahl.ForEach(t => clbxMitarbeiterFilter.Items.Add($"{t.Name} | {t.Personalnummer}", true));
  }
  ...
  {
    return absences.ToList().Where(abwesenheit => spezifikationen.IsSatisfiedBy(abwesenheit)).ToImmutableList();
  }

  private void AbwesenheitenExportieren(IImmutableList<Abwesenheitseintrag<TSource, TTarget>> gefilterteAbwesenheiten)
  {
    var exportAbsences = gefilterteAbwesenheiten.Where(a => _mappingControl.IsExport(a.AusgangsTyp)).ToList();

    var absencesMappedToTarget = _mappingControl.MapAbsencesToTarget(exportAbsences);
  ...
  {
    var bindingList = new SortableBindingList<Abwesenheitseintrag<TSource, TTarget>>(absences.ToList());
  ...
  private void TabellenEinstellungenAnwenden()
  {
    ...
    dgvDateneintraege.Columns[1].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
    dgvDateneintraege.Columns[6].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
    dgvDateneintraege.Columns[7].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

    dgvDateneintraege.Sort(dgvDateneintraege.Columns[1], ListSortDirection.Ascending);
  }

  private async Task<IImmutableList<Abwesenheitseintrag<TSource, TTarget>>> AbwesenheitenAbfragen(
    HashSet<TSource> absenceTypes
  )
  {
    ...
    var absencesWithMapping = _mappingControl.MapAbsencesFromSource(await absencesWithoutMapping);
    var absencesThatAreNotIgnored = absencesWithMapping.Where(absence => absence.ZielTyp is not null).ToImmutableList();
    return absencesThatAreNotIgnored;
  }

  private IImmutableList<Abwesenheitseintrag<TSource, TTarget>> GefilterteAbwesenheitenErhalten()
  {
    ...
    var gefilterteAbwesenheiten = AbwesenheitenFiltern(Abwesenheiten, mitarbeiterUndKrankheitsFilter);
