...
public class GdiAbwesenheitDataTableRepository : IGdiAbwesenheitRepository
{
  ...
  public async Task<Result<HashSet<GdiAbsenceType>>> GetAll(CancellationToken cancellationToken = default)
  {
    var abwesenheiten = new HashSet<GdiAbsenceType>();

    try
    {
      var kundenAbfrage = _tableClient.QueryAsync<GdiAbwesenheitsTypEntity>(cancellationToken: cancellationToken);
  ...
  )
  {
    var upsertActions = gdiAbwesenheitsTypen
      ...
      .Select(kundeRelationDto => new TableTransactionAction(TableTransactionActionType.UpsertMerge, kundeRelationDto));
