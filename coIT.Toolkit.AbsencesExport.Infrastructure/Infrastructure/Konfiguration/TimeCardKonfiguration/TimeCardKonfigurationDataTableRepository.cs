...
public class TimeCardKonfigurationDataTableRepository : ITimeCardKonfigurationRepository
{
  ...
  public TimeCardKonfigurationDataTableRepository(string connectionString, IDoCryptography cryptographyService)
  {
    _mapper = new TimeCardKonfigurationMapper(cryptographyService);
    _tableClient = new TableClient(connectionString, TimeCardKonfigurationEntity.TabellenName);
  }

  public async Task<Result<TimeCardSettings>> Get(CancellationToken cancellationToken = default)
  {
    try
    {
      return await Result
    ...
    try
    {
      return await _mapper
        ...
        .Tap(konfigurationEntity =>
          _tableClient.UpsertEntityAsync(konfigurationEntity, cancellationToken: cancellationToken)
        );
    }
