...
public class ClockodoAbwesenheitsTypeDataTableRespository : IClockodoAbwesenheitsTypRepository
{
  ...
  {
    _tableClient = new TableClient(connectionString, ClockodoAbwesenheitsTypEntity.TabellenName);
  }

  public async Task<Result<HashSet<ClockodoAbsenceType>>> GetAll(CancellationToken cancellationToken = default)
  {
    var abwesenheiten = new HashSet<ClockodoAbsenceType>();

    try
    {
      var clockodoAbwesenheitsTypenAbfrage = _tableClient.QueryAsync<ClockodoAbwesenheitsTypEntity>(
        cancellationToken: cancellationToken
      );

      await foreach (var clockodoAbwesenheitsTypEntity in clockodoAbwesenheitsTypenAbfrage)
      {
        var clockodoAbwesenheitsTyp = ClockodoAbwesenheitsTypMapper.ZuClockodoAbwesenheitsTyp(
  ...
  )
  {
    ...
    try
    {
      ...
      var antworten = await _tableClient.SubmitTransactionAsync(upsertActions).ConfigureAwait(false);
